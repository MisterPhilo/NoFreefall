...
function Module:ScatterOut(Part, rLimit, vLimit, ...)
	if IsAPart(Part) then
		...
		local Tween = TS:Create(Part, TweeningInfo(...), {
			CFrame = Part.CFrame
				* F.ANG(F.rad(RandDec(unpack(rLimit))), F.rad(RandDec(unpack(rLimit))), F.rad(RandDec(unpack(rLimit))))
				* F.CN(RandDec(unpack(vLimit)), RandDec(unpack(vLimit)), RandDec(unpack(vLimit))),
		})
...
function Module:CompleteScatterOut(Part, Xlimit, Ylimit, Zlimit, Rlimit, ...) -- Like :ScatterOut but control over all CFrame values.
	if IsAPart(Part) then
		...
		local Tween = TS:Create(Part, TweeningInfo(...), {
			CFrame = Part.CFrame
				* F.CN(RandDec(unpack(Xlimit)), RandDec(unpack(Ylimit)), RandDec(unpack(Zlimit)))
				* F.ANG(F.rad(RandDec(unpack(Rlimit))), F.rad(RandDec(unpack(Rlimit))), F.rad(RandDec(unpack(Rlimit)))),
		})
		if self.Autoplay then
			Tween:Play()
